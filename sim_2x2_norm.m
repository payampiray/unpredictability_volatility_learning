...
if do_sim
    config = struct('x0_unc',100,'sigma_v',.1,'sigma_u',.1,'v0',1,'u0',2,'nparticles',100,...
    ...
    for j=1:4
        ...
        for i=1:nsim
            ...
            [vol(:,i),unp(:,i),lr(:,i),val(:,i)]=model(outcome(:,i),config);
        end
        outcomes{j} = outcome;        
...
function [vol,unp,lr,val,unc] = model(o,specs)
np = specs.nparticles;
...
v = specs.v0;
u = specs.u0;
[val,vol,unp,lr,unc] = pf_core_norm(o,x0_unc,sigma_v,sigma_u,v,u,np);        
end